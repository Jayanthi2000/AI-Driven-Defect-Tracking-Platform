// TesterNavbar.jsx — matches AdminNavbar exactly
import { useState, useRef, useEffect } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { getCurrentUser } from '../../services/testerService';
import { getNotificationsByUser, markAllAsRead, markAsRead } from '../../services/notificationService';
...
const PAGE_TITLES = {
  ...
  '/tester/report-bug':  'Report Bug',
  '/tester/my-bugs':     'My Bugs',
  '/tester/activity':    'Activity Log',
  '/tester/profile':     'Profile',
};

export default function TesterNavbar({ onMenuClick }) {
  ...
  const title = PAGE_TITLES[location.pathname] ||
    (location.pathname.startsWith('/tester/bug-details') ? 'Bug Details' : 'Tester');

  useEffect(() => {
    if (user) setNotifications(getNotificationsByUser(user.id).slice(0, 10));
  }, [location.pathname]);
  ...
  const handleNotifClick = (notif) => {
    ...
    if (notif.bugId) { navigate(`/tester/bug-details/${notif.bugId}`); setNotifOpen(false); }
  };
      ...
      <div className="max-w-screen-xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#6b7280] text-xs hidden sm:block">DefectAI Tester Module</p>
          </div>
        </div>
                      ...
                      notifications.map(n => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-[#4b5563] mt-1">{timeAgo(n.createdAt)}</p>
                          </div>
                        </button>
                      ))
                    )}
          ...
          <div ref={profileRef} className="relative">
            ...
            >
              ...
              <div className="hidden sm:block text-left">
                <p className="text-xs font-medium text-white leading-none">{(user?.name || 'Tester').split(' ')[0]}</p>
                <p className="text-[10px] text-[#22c55e] capitalize mt-0.5">Tester</p>
              </div>
            ...
            <AnimatePresence>
              {profileOpen && (
                ...
                >
                  ...
                  <button
                    onClick={() => { navigate('/tester/profile'); setProfileOpen(false); }}
